   ...
   public function create(Request $request)
    {
        ...
        if ($request->wantsJson()) {
            return response()->json(['doctorsList' => $doctorsList]);
        }
    ...
    public function store(CreateDoctorSessionRequest $request)
    {
        ...
        if ($request->wantsJson()) {
            return response()->json(['message' => __('messages.flash.schedule_crete')]);
        }
     ...
     */
    public function doctorScheduleEdit(Request $request)
    {
        $doctorSession = DoctorSession::whereDoctorId(getLogInUser()->doctor->id)->first();

        if (empty($doctorSession)) {
            if ($request->wantsJson()) {
                return response()->json(['error' => __('messages.flash.schedule_not_found')], 404);
            } else {
                flash(__('messages.flash.schedule_not_found'))->error();
                return redirect(route('doctor-sessions.index'));
            }
        }
